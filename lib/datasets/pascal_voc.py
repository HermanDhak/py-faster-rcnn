...
class pascal_voc(datasets.imdb):
    def __init__(self, image_set, devkit_path=None):
        ...
        self._image_ext = '.png'
        self._image_index = self._load_image_set_index('train_index.txt')
    ...
    def _load_annotation(self):
        ...
        """
        gt_roidb = []
        annotationfile = os.path.join(self._data_path, 'train.txt')
        ...
        while(split_line):
            ...
            for i in range(num_objs):
                x1 = float (split_line[2 + i * 4]) -1
                y1 = float (split_line[3 + i * 4]) -1
                x2 = float (split_line[4 + i * 4]) -1
                y2 = float (split_line[5 + i * 4]) -1
                cls = self._class_to_ind['foreground']
                boxes[i,:] = [x1, y1, x2, y2]
                gt_classes[i] = cls
                overlaps[i,cls] = 1.0
